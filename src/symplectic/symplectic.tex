...
\begin{align}
  ...
  \notag
\end{align}
また, 途中の計算において$\partial_{q_{i}} \mathcal{H}$と$\partial_{q_{j}} \mathcal{H}$が可換であることと, $p_{i},~p_{j},p_{k}$に関する偏微分の順番を入れ替えられることを用いた.
...
\begin{align}
  ...
  \notag
\end{align}
が得られた. 第3項目1式の$i$と$j$を置換して式を整理すると,
\begin{equation}
  -
  \partial_{q_{j}}       \mathcal{H}
  \partial_{p_{i} p_{j}} \mathcal{H}
...
更に, 第2項目3式の$i$, $j$を置換した後に$j$, $k$を置換すると
\begin{align}
  -
  \partial_{q_{j}} \mathcal{H}
  \partial_{p_{i}} \mathcal{H}
...
{$[D_{U} , [D_{U} , D_{K}]]$の計算過程と同じように頑張れば得られる.
